...
namespace Globals
{
	...
	matrix3x4_t desyncMatrix[128];
	bool get_fake_matrix;
	Vector3 fakeLagOrigin;
	CBaseEntity* localPlayer;
	Vector3 lastgoodimpact;
	float autowallDmg;
	...
	bool Hit[ 65 ];
	int MissedShots[65];
	std::vector<matrices_s> m_vecMatrices;
	bool inThirdperson;
	bool breakingLC;
	Vector3 fakeAngle;
	Vector3 fakeAngle2;
...
/*bool c_config::save(std::string file_name) {
	CreateDirectory("C:\\catsense\\configs\\", NULL); CreateDirectory("C:\catsense\\configs\\", NULL);
	std::ofstream ofs("C:\\catsense\\configs\\" + file_name + ".cfg");
}*/

/*bool c_config::load(std::string file_name)
{
	CreateDirectory("C:\\catsense\\configs", NULL);

	std::string file_path = "C:\\catsense\\configs\\" + file_name + ".cfg";
...
void c_config::save(const std::string& file_name) {
	...
	//if (res == S_OK)
	{
		auto mmeme = fs::path("C:\\Exp0sure\\configs") / file_name;
...
void c_config::load(const std::string& file_name) {
	...
	//if (res == S_OK)
	{
		if (fs::exists(fs::path("C:\\catsense\\configs") / file_name)) {
			std::string loaderConfig;
			std::ifstream(fs::path("C:\\catsense\\configs") / file_name, std::ios::binary) >> loaderConfig;
