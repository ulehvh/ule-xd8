...
void ChangeRegion() {
	static std::string* ForceRelayClusterValue = *(std::string**)(g_Memory->findPattern("steamnetworkingsockets.dll", "B8 ? ? ? ? B9 ? ? ? ? 0F 43") + 1);

	*ForceRelayClusterValue = DatacentersList[config->get_int("miscRegionChanger")];
}


void menu_setup::setup() {
	static std::vector<std::string> WavItems = menu->GetAllWavs();
	Globals::SoundFiles = WavItems;
	config->set("clr_menu", Color(59, 81, 245));
	...
	*/
	...
	c_tab* legit_tab = new c_tab("Legit-bot");
	{
		...
		{
			main_group->add_child(new c_checkbox("Enable", "legitEnable"));
			main_group->add_child(new c_checkbox("Legit Aimbot", "legitAim"));
			main_group->add_child(new c_hotkey("Aim on Key", "legitKey", true));
			main_group->add_child(new c_checkbox("Aim on Key", "legitOnKey"));
			main_group->add_child(new c_slider("FOV", "legitFOV", 0, 30, true, L"%.0f"));
			main_group->add_child(new c_slider("Smoothing", "legitSmooth", 1, 100, true, L"%.0f"));
			main_group->add_child(new c_combo("Hitbox", "legitBone", {"Closest to crosshair","Head", "Neck", "Pelvis", "Chest"}));
			main_group->add_child(new c_checkbox("Silent Aim", "SilentAim"));
			main_group->add_child(new c_checkbox("Recoil Control System", "RCSlegit"));
		}
	...
	c_tab *rage_tab = new c_tab("Rage-bot");
	{
		...
		c_group *general_group = new c_group("Aimbot", Vector2(223, 436));
		{
			...
			//general_group->add_child(new c_checkbox("Auto-Revolver", "autoRevolver"));
	...
	c_tab* antiaim_tab = new c_tab("Anti-aim");
	{
		...
		//	antiaim_settings_group->add_child(new c_checkbox("Desync", "aaDesync"));
			antiaim_settings_group->add_child(new c_checkbox("Desync", "taaDesync"));
			antiaim_settings_group->add_child(new c_checkbox("Break LBY", "aaDesyncLby"));
	...
	c_tab *visuals_tab = new c_tab("Visuals");
	{
		...
		c_group *player_group = new c_group("Player ESP", Vector2(223, 230));
		{
			...
			player_group->add_child(new c_colorpicker("TracerColor", "colorBulletTracerLocal", true, true));
			player_group->add_child(new c_checkbox("Local Bullet tracers", "miscBulletTracerLocal"));
			player_group->add_child(new c_colorpicker("TracerColorEnemy", "colorBulletTracerEnemy", true, true));
			player_group->add_child(new c_checkbox("Enemy Bullet tracers", "miscBulletTracerEnemy"));
			...
			player_group->add_child(new c_checkbox("Anti-aim indicators", "AAind"));
			
			player_group->add_child(new c_checkbox("Hit marker", "espHitmarker"));

			if (!WavItems.empty()) {
				player_group->add_child(new c_checkbox("Hit Sounds", "espHitmarkerSoundEnable"));
				player_group->add_child(new c_combo("Sound", "espHitmarkerSound", WavItems));
			}
			else {
				player_group->add_child(new c_checkbox("Hit Sound (resources folder)", "espHitmarkerSoundEnable"));
			}
		...
		{
			models_group->add_child(new c_combo("Enemy material", "chamsMaterial", { "Textured", "Reflective", "Flat", "Astro" ,"Animated"}));
			models_group->add_child(new c_combo("Local material", "localchamsMaterial", { "Textured", "Reflective", "Flat", "Astro","Animated" }));
			models_group->add_child(new c_combo("View-model material", "vmchamsMaterial", { "Textured", "Reflective", "Flat", "Astro","Animated" }));
			//models_group->add_child(new c_combo("Team-mate model material",  "teamchamsMaterial", { "Textured", "Reflective", "Flat", "Glow" }));

		
			
			models_group->add_child(new c_colorpicker("xqzchamsclr", "colorChamsEnemyXQZ", true, true));
			models_group->add_child(new c_checkbox("Player", "chamsEnemyXQZ"));

			models_group->add_child(new c_colorpicker("Enemy Pearlescence", "enemyPearlColor", true, false));
			models_group->add_child(new c_checkbox("Enemy Pearlescence", "enemyPearl"));
			...
			models_group->add_child(new c_colorpicker("hitmarkerchams", "colorHitmarkerChams", true, false));
			models_group->add_child(new c_checkbox("Hitmarker Chams", "hitChams"));
			...
			models_group->add_child(new c_colorpicker("Local Pearlescence", "localPearlColor", true, false));
			models_group->add_child(new c_checkbox("Local Pearlescence", "localPearl"));
		


			models_group->add_child(new c_colorpicker("clrDesync", "colorDesyncModel", true, true));
			models_group->add_child(new c_checkbox("Desync chams", "chamsLocalDesync"));
		}
		...
		c_group *effects_group = new c_group("Effects", Vector2(223, 210));
		{
			...
			effects_group->add_child(new c_slider("Prop Transparency", "propAlpha", 0, 100, true, "%.0f%%"));
	...
	c_tab *misc_tab = new c_tab("Miscellaneous");
	{
		...
		c_group *misc_group = new c_group("Miscellaneous", Vector2(223, 436));
		{
			...
			misc_group->add_child(new c_hotkey("Block-bot", "BlockbotHkey", true));
			misc_group->add_child(new c_checkbox("Block-bot", "miscBlockBot"));

			misc_group->add_child(new c_hotkey("freecamKey", "freecamKey", true));
			misc_group->add_child(new c_checkbox("Freecam (Use WASD)", "miscFreecam"));
			misc_group->add_child(new c_slider("Freecam Speed", "freecamSpeed", 0, 10, true, L"%.0f"));
			...
			misc_group->add_child(new c_checkbox("Unlimited Duck", "miscUnlimitedDuck"));
			misc_group->add_child(new c_combo("Model Changer", "miscMdlChanger", { "Default","Special Agent Ava","Operator","Markus Delrow","Michael Syfers","B Squadron Officer","Seal Team 6 Soldier","Buckshot","Lt. Commander Ricksaw","Third Commando Company","'Two Times' McCoy","Dragomir","Rezan The Ready","'The Doctor' Romanov","Maximus","Blackwolf","The Elite Mr. Muhlik","Ground Rebel","Osiris","Prof. Shahmat","Enforcer","Slingshot","Soldier" }));

			misc_group->add_child(new c_combo("Region changer", "miscRegionChanger", { "Default", "Australia", "Austria", "Brazil", "Chile", "Dubai", "France", "Germany", "Hong Kong", "India (Chennai)", "India (Mumbai)", "Japan", "Luxemburgo","Netherlands", "Peru", "Philipines", "Poland", "Singapor", "South Africa", "Spain", "Sweden", "UK", "USA (Atlanta)", "USA (Bellevue)", "USA (Chiraq)", "USA (Los Angeles)", "USA (Moses Lake)","USA (Oklahoma)", "USA (Seattle)", "USA (Washinton DC)" }));

			misc_group->add_child(new c_button("Change Region", []() {
				ChangeRegion();
				}));
		...
		{
			other_group->add_child(new c_combo("Presets", "cfgPreset", { "Legit", "Rage", "HvH", "Secret", "Headshot", "Scout", "Auto", "Pistol", "Heavy Pistol" }));
			...
			other_group->add_child(new c_button("Load config", []() {
				...
				config->set("espHitmarkerSound", 0);
			...
			other_group->add_child(new c_button("Open Config folder", []() {
				ShellExecute(NULL, "open", "C:\\catsense", NULL, NULL, SW_SHOWDEFAULT);
				}));
			other_group->add_child(new c_button("Bypass sv_cheats", []() {
				ShellExecute(NULL, "open", "https://www.youtube.com/watch?v=dQw4w9WgXcQ", NULL, NULL, SW_SHOWDEFAULT);
				//ShellExecute(NULL, "open", "https://www.youtube.com/watch?v=dQw4w9WgXcQ", NULL, NULL, SW_SHOWDEFAULT);
				}));
		}
