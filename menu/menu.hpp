...
public:
	...
	SoundFile(const std::string name, const std::string path)
	{
		this->name = name;
		this->path = path;
	}
...
public:
	...
	std::vector<std::string> GetAllWavs();

	std::vector<SoundFile> GetAllConfigsInFolder(const std::string path, const std::string ext);
	...
	bool get_hotkey(const std::string& value, int type);
