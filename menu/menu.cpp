...
std::vector<std::string> c_menu::GetAllWavs()
{
	namespace fs = std::experimental::filesystem;

	std::string fPath = std::string("C:\\catsense\\resources");

	std::vector<SoundFile> sound_files = GetAllConfigsInFolder(fPath, ".wav");
	std::vector<std::string> sound_file_names;

	for (auto config = sound_files.begin(); config != sound_files.end(); config++)
		sound_file_names.emplace_back(config->GetName());

	std::sort(sound_file_names.begin(), sound_file_names.end());

	return sound_file_names;
}

std::vector<SoundFile> c_menu::GetAllConfigsInFolder(const std::string path, const std::string ext)
{
	namespace fs = std::experimental::filesystem;
	...
	if (fs::exists(path) && fs::is_directory(path))
	{
		for (auto it = fs::recursive_directory_iterator(path); it != fs::recursive_directory_iterator(); it++)
		{
			if (fs::is_regular_file(*it) && it->path().extension() == ext)
			{
				...
				SoundFile new_config(fName, fPath);
				sound_files.emplace_back(new_config);
			}
		}
	}
	return sound_files;
...
bool c_menu::get_hotkey(const std::string& value, int type) {
	int key = config->get_int(value);
	int activation_type = type;
...
void c_menu::think( ) {
	...
	if ( this->is_hovered( fixed_position - Vector2( 6, 6 ), fixed_position + Vector2( size.x + 6, 6 ) ) ) {
		...
		for ( int i = 0; i < ( int )tabs.size( ); i++ ) {
			...
			const int tab_width = (Exp0sureTabSize / ( int )tabs.size( ) );
...
void c_menu::draw( ) {
	...
	//g_renderer_d3d->rect_fill( fixed_position.x + 1, fixed_position.y + 1, size.x - 2, 2,  Color(59, 81, 245) );
	g_renderer_d3d->gradient_h(fixed_position.x + 1, fixed_position.y + 1, size.x - 2, 2, Color(59, 81, 245), Color(212, 108, 252));
	g_renderer_d3d->line( fixed_position.x + 1, fixed_position.y + 2, fixed_position.x + size.x - 1, fixed_position.y + 2,
		Color( 0, 0, 0, ( int )( 80.0f * ( ( float ) 1.f ) ) ) );

	g_renderer_d3d->rect_fill( fixed_position.x + 19, fixed_position.y + this->size.y - 40, Exp0sureTabSize, 25, Color( 24, 24, 24 ) );
	g_renderer_d3d->rect( fixed_position.x + 18, fixed_position.y + this->size.y - 41, Exp0sureTabSize + 2, 27, Color( 44, 44, 44 ) );

	for ( int i = 0; i < ( int )tabs.size( ); i++ ) {
		c_tab *tab = tabs[ i ];

		auto tab_text_color = tab == active_tab ?  Color(59, 81, 245) : Color( 210, 210, 210 );
		auto tab_text_font = tab == active_tab ? g_renderer_d3d->menu_bold : g_renderer_d3d->menu;

		const int tab_width = (catsenseTabSize / ( int )tabs.size( ) );
