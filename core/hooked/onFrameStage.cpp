		...
		if ( frameStage == FRAME_RENDER_START )
		{
			...
			for ( int i = 1; i < g_Interfaces->globalVars->maxClients; ++i )
			{
				...
				if (config->get_bool("rageAntiAimCorrection")) {
					//g_Resolver.HandleHits(pEntity);
					g_Resolver.preresolver(pEntity->animState(), pEntity, false);
					//g_Resolver.PostResolver(pEntity->animState(), pEntity, false);
					//g_Resolver.AnimationFix(pEntity);
				}
