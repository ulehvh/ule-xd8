...
{
	// this is where we hook shit if u didn't know. basically, slay all  ~alpha
	player_hurt::get( ).registerSelf( );
	bullet_impact::get( ).registerSelf( );
	weapon_fire::get( ).registerSelf( );
	player_death::get( ).registerSelf( );
	//auto asd = g_Interfaces->EngineClient->GetBSPTreeQuery();
	...
	//this->listLeavesHook = new CVFTHook(asd);
	...
	//this->listLeavesHook->swapFunction(6, CHookManager::hkListLeavesInBox);
