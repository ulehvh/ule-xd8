...
void CAimbot::legitCreateMove() {
	
	if (!config->get_bool("legitEnable"))
		return;

	if (config->get_bool("legitAim"))
		legitAim();
...
void CAimbot::creatMove()
{
	...
	if (config->get_bool("autoRevolver"))
		autoRevolver();
...
void CAimbot::autoRevolver() {
	...
	auto local_weapon = Globals::localPlayer->activeWeapon();
	if (!local_weapon)
		return;
	...
	if (local_weapon) {
		...
		if (!local_weapon->isReloading()) {


			r8cock_flag = true;

			if (r8cock_flag && canShoot)
			{
				if (r8cock_time <= TICKS_TO_TIME(Globals::localPlayer->tickBase()))
				{
					if (Globals::localPlayer->activeWeapon()->nextSecondaryAttack() <= TICKS_TO_TIME(Globals::localPlayer->tickBase()))
						r8cock_time = TICKS_TO_TIME(Globals::localPlayer->tickBase()) + 0.234375f;
					else
						Globals::oCmd->buttons |= IN_ATTACK2;
				}
				else
					Globals::oCmd->buttons |= IN_ATTACK;

				r8cock_flag = TICKS_TO_TIME(Globals::localPlayer->tickBase()) > r8cock_time;
			}
			else
			{
				r8cock_flag = false;
...
Vector3 m_oldangle;
Vector3 RealAngle;
void CAimbot::legitRcs() {
	if (!config->get_bool("RCSlegit") )
		return;
	
	auto weap = Globals::localPlayer->activeWeapon();


	if (Globals::oCmd->buttons & IN_ATTACK)
	{
		Vector3 aimpunch = Globals::localPlayer->aimPunchAngle() * 2.f;

		Globals::oCmd->viewAngles -=  aimpunch;
...
bool CheckTarget(CBaseEntity* entity,Vector3 aimpoint) {
	CGameTrace tr;
	C_Ray ray;
	CTraceFilter filter;
	filter.pSkip1 = Globals::localPlayer;
	auto start = Globals::localPlayer->eyePosition();


	ray.Init(start, aimpoint);
	g_Interfaces->trace->TraceRay(ray, MASK_SHOT | CONTENTS_GRATE, &filter, &tr);
...
int ClosestBone(CBaseEntity* Entity)
{
	...
	if (!Entity->SetupBones(matrix, 128, 0x00000100, g_Interfaces->globalVars->curtime))
		return -1;

	studiohdr_t* pStudioModel = g_Interfaces->modelInfo->getStudiomodel(Entity->GetModel());
	if (!pStudioModel)
		return -1;

	mstudiohitboxset_t* set = pStudioModel->getHitboxSet(Entity->hitboxSet());
	if (!set)
		return -1;
	...
	for (int i = 0; i < set->numhitboxes; i++)
	{
		...
		auto thisdist = g_Math.GetFov(Globals::oCmd->viewAngles, g_Math.calcAngle(Globals::localPlayer->eyePosition(), Vector3(matrix[hitbox->bone][0][3], matrix[hitbox->bone][1][3], matrix[hitbox->bone][2][3])));
...
void CAimbot::legitAim() {
	...
	for (int i = 0; i <= g_Interfaces->globalVars->maxClients; i++) {
		...
		if (!pEntity
			|| !pEntity->alive()
			|| pEntity->IsDormant()
			|| pEntity->team() == Globals::localPlayer->team()
			|| pEntity == Globals::localPlayer
			|| pEntity->immunity())
		{
			continue;
		}
		...
		Vector3 aimPunchAngle = Globals::localPlayer->aimPunchAngle() * 2.f;
	

		Vector3 origin = Globals::localPlayer->GetBonePosition(8), point =  pEntity->GetBonePosition(GetBone(config->get_int("legitBone")));

		if (!CheckTarget(pEntity, point))
			continue;

		Vector3 enemy_direction = g_Math.calcAngle(origin, point);
		enemy_direction.x -= aimPunchAngle.x;
		enemy_direction.y -= aimPunchAngle.y;

		//g_Math.vectorNormalize(enemy_direction);
		
		

		if ((Globals::oCmd->viewAngles - enemy_direction).Length2D() <= config->get_float("legitFOV") && enemy_direction != Vector3(0, 0, 0) ) {
			g_Math.SmoothAngle(Globals::oCmd->viewAngles, enemy_direction, config->get_float("legitSmooth")); 
			if (Globals::oCmd->buttons & IN_ATTACK || (config->get_bool("legitOnKey") && menu->get_hotkey("legitKey"))) {
				Globals::oCmd->viewAngles = enemy_direction;
				if (!config->get_bool("SilentAim"))
					g_Interfaces->gameEngine->setViewAngles(Globals::oCmd->viewAngles);
...
Vector3 CAimbot::hitScan(CBaseEntity* pEnt) // supremeemmemememememe
{
	...
	for (int hitbox : Scan)
	{
		...
		if (config->get_bool("rageBodyAimLethal") && hitbox != 0 && hitbox != 25 && hitbox != 26 && hitbox != 27 && Damage >= (pEnt->health()))
		{
